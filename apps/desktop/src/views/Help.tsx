  ...
  return (
    <article className="overflow-hidden rounded-lg border border-border bg-panel">
      <button onClick={onToggle} aria-expanded={open} className={`flex w-full items-start gap-3 px-4 py-3 text-left hover:bg-hover ${FOCUS_RING}`}>
        <ChevronRight size={16} className={`mt-0.5 shrink-0 text-muted transition-transform ${open ? "rotate-90" : ""}`} />
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-4xl flex-col gap-8 p-6">
        ...
        {resultats.length === 0 && <p className="p-8 text-center text-sm text-muted">Aucune fiche ne correspond à « {query} ».</p>}
      </div>
    </PageLayout>
  );
}
