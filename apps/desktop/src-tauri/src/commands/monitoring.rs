...
#[derive(Default)]
pub struct Monitor {
    prev: Mutex<HashMap<String, RawSample>>,
}
...
#[tauri::command]
pub async fn agent_install(store: State<'_, Store>, sessions: State<'_, Sessions>, server_id: String) -> Result<String, String> {
    let (conn, pw) = admin(&store, &sessions, &server_id).await?;
...
#[tauri::command]
pub async fn agent_uninstall(store: State<'_, Store>, sessions: State<'_, Sessions>, server_id: String) -> Result<(), String> {
    let (conn, pw) = admin(&store, &sessions, &server_id).await?;
