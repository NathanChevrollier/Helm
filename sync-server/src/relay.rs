...
pub struct Room {
    ...
    host_inbox: Mutex<Option<mpsc::UnboundedReceiver<String>>>,
    created: Instant,
...
pub async fn create(State(app): State<Arc<App>>, headers: HeaderMap) -> Response {
    ...
    app.rooms.0.lock().await.insert(id.clone(), Arc::new(room));
...
async fn host_loop(socket: WebSocket, room: Arc<Room>, mut inbox: mpsc::UnboundedReceiver<String>, app: Arc<App>, session: String) {
...
async fn guest_loop(socket: WebSocket, room: Arc<Room>) {
    ...
    loop {
        tokio::select! {
            incoming = rx.next() => match incoming {
                Some(Ok(Message::Text(text))) => {
                    if room.to_host.send(text.to_string()).is_err() {
                        break;
                    }
                }
                Some(Ok(Message::Ping(_) | Message::Pong(_) | Message::Binary(_))) => {}
